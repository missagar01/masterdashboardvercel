...
import { MAINTENANCE_API_BASE } from "@/lib/apiEndpoints";

const MachineMaintenanceDashboard = () => {
  ...
  // Use the Next.js API proxy (/api/dashboard/*) to bypass browser CORS.
  const BACKEND_URL = `${MAINTENANCE_API_BASE}/api`;
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        setErrorMessage(
          "Could not load machine maintenance data (likely CORS from the API). Data shown is empty."
        );
        setSheetData([]);
        setMaintenanceTasks([]);
        setMaintenanceCostData([]);
        setDepartmentCostData([]);
        setFrequentRepairData([]);
      }
    };
